...
*Importantly, no global references to the secret object are retained by the `private` package, so as soon as `obj` gets garbage collected, the secret will be reclaimed as well. In other words, you don't have to worry about.jsx memory leaks.*
...
```
You can learn something about.jsx `secret` by calling `.increment()`, and you
